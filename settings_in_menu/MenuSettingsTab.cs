...
private void DrawMenuTab()
        {
            ...
            GUILayout.Label(L("FPS Limit", "Лимит FPS"), toggleLabelStyle, GUILayout.Height(25), GUILayout.Width(110));
            ...
            GUILayout.BeginHorizontal();
            GUILayout.Label(L("Accent Color", "Цвет акцента"), toggleLabelStyle, GUILayout.Height(25), GUILayout.Width(110));
            ...
            GUILayout.Label(L("Fake Name", "Поддельное имя"), toggleLabelStyle, GUILayout.Height(20), GUILayout.ExpandWidth(false));
            GUILayout.Space(3);
            ...
            GUILayout.Label(L("Fake RPC sends the selected non-vanilla CallRpc as your local player. Custom RPC accepts only IDs outside the vanilla RPC list.",
                "Fake RPC отправляет выбранный не-ванильный CallRpc от вашего игрока. Custom RPC принимает только ID вне списка ванильных RPC."), clippedMenuDescStyle, GUILayout.Height(36f));
            GUILayout.EndVertical();
            ...
            GUILayout.Space(6);
            GUILayout.BeginHorizontal();
            if (GUILayout.Button(L("Reset Sliders to Default", "Сбросить ползунки до дефолта"), activeTabStyle, GUILayout.Height(30), GUILayout.ExpandWidth(true)))
            {
                ResetSlidersToDefault();
                menuPrefsChanged = true;
            }
            GUILayout.Space(6);
            if (GUILayout.Button(L("Reset Menu Size", "Сбросить размер меню"), btnStyle, GUILayout.Height(30), GUILayout.ExpandWidth(true)))
            {
                ResetMenuSize();
                menuPrefsChanged = true;
            }
...
private void ResetSlidersToDefault()
        {
            ...
            replayOverlayOnRadar = false;
            replayHideRadarLive = false;
            replayOnlyLastSeconds = true;
            replaySeconds = 30f;
            replayDrawIcons = false;
            replayFilterMask = 0x3FF;
            replayRect = new Rect(250f, 90f, 640f, 440f);
            replayLogRect = new Rect(760f, 90f, 430f, 360f);
